...
tic;
[optimTraj,fval,exitflag,output,lambda,grad,hessian] = fmincon(@cartpoleObFun,optimInitGuess,A,B,Aeq,Beq,LB,UB,@cartpoleConstraints2,options,h,params);
toc;

%%
%double-check to see if dynamics were enforced
[tcheck,ycheck] = cartpolePlayUtape(tcp,optimTraj(3,:));    
%the double-check may not work because it's open-loop...any little
%deviation sends it off course...

%%
%double-check to see if constraints were enforced
constraintCheck = cartpoleCheckOptimTraj(optimTraj,h,params);

%%
figure;
hold on;
plot(tcp,ycp(1,:),'k.-','LineWidth',2);
